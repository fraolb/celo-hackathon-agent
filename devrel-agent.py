...
def get_user_input() -> Dict[str, Any]:
    ...
    if source_answer["source"] == "excel":
        ...
        github_url_question = [
            inquirer.Text(
                "github_urls", message="Enter GitHub URLs (comma-separated):"
            ),
        ]
    ...
    common_questions = [
        ...
        inquirer.Text(
            "config_path", message="Enter config file path:", default="config.json"
        ),
    ]
...
def create_dataframe_from_urls(
    github_urls: str, project_name: str, project_description: str
) -> pd.DataFrame:
    """
...
def run_analysis(user_input: Dict[str, Any]) -> int:
    ...
    try:
        ...
        if api_key_env_var and not os.environ.get(api_key_env_var):
            ...
            print(
                f"Please set the {api_key_env_var} environment variable in your .env file."
            )
            return 1
        ...
        print(
            f"  {COLORS['cyan']}🤖 Model Provider:{COLORS['reset']} {model_provider.capitalize()}"
        )
        print(
            f"  {COLORS['cyan']}📊 Input Source:{COLORS['reset']} {user_input['source']}"
        )
        if user_input["source"] == "excel":
            print(
                f"  {COLORS['cyan']}📄 Excel Path:{COLORS['reset']} {user_input['excel_path']}"
            )
        else:
            print(
                f"  {COLORS['cyan']}🔗 GitHub URLs:{COLORS['reset']} {user_input['github_urls']}"
            )
            print(
                f"  {COLORS['cyan']}📝 Project Name:{COLORS['reset']} {user_input['project_name']}"
            )
        print(
            f"  {COLORS['cyan']}⚙️  Config:{COLORS['reset']} {user_input['config_path']}"
        )
        print(
            f"  {COLORS['cyan']}📁 Output:{COLORS['reset']} {user_input['output_dir']}"
        )
        print(
            f"  {COLORS['cyan']}🔊 Verbose:{COLORS['reset']} {'Yes' if user_input['verbose'] else 'No'}"
        )
        ...
        # Generate reports
        logger.info(
            f"Generating reports for {project_count} projects", step="Report Generation"
        )
        generate_reports(results, user_input["output_dir"])
        ...
        print(
            f"  {COLORS['cyan']}📊 Projects Analyzed:{COLORS['reset']} {project_count}"
        )
        print(
            f"  {COLORS['cyan']}📁 Reports:{COLORS['reset']} {user_input['output_dir']}/"
        )
...
def main():
    ...
    parser.add_argument(
        "--urls", help="Comma-separated GitHub URLs for direct analysis"
    )
    parser.add_argument(
        "--project-name",
    ...
    parser.add_argument(
        "--project-desc", default="", help="Project description for direct URL analysis"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help="AI model provider (anthropic, openai, google)",
    )
    ...
    if args.non_interactive:
        ...
        if args.excel:
            ...
            print(
                "Error: In non-interactive mode, you must provide either --excel or --urls"
            )
            return 1
    else:
        # Get user input through interactive prompts
