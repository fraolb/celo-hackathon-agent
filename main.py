...
class Spinner:
    def __init__(self, message="Loading"):
        ...
        self.fallback_frames = [
            ...
            "|",
            "/",
        ]  # Fallback if unicode doesn't display well
        self.delay = 0.1
    ...
    def update(self, message=None):
        ...
        spinner = f"{self.colors['cyan']}{self.current_frames[self.index]}{self.colors['reset']}"
        formatted_message = (
            f"{self.colors['bold']}{display_message}{self.colors['reset']}"
        )
    ...
    def stop(self, message=None):
        ...
        if (
            "Error" in final_message
            or "error" in final_message
            or "failed" in final_message
        ):
            icon = (
                f"{self.colors['yellow']}⚠️{self.colors['reset']}"  # Warning for errors
            )
        else:
    ...
    try:
        ...
        with open(summary_path, "w") as f:
            ...
            for result in results:
                ...
                if "analysis" in result and "code_quality" in result["analysis"]:
                    ...
                    if (
                        isinstance(code_quality, dict)
                        and "overall_score" in code_quality
                    ):
                        ...
                        score_display = (
                            f"{score:.2f}"
                            if repos_analyzed <= 1
                            else f"{score:.2f} ({repos_analyzed} repos)"
                        )
                ...
                if "analysis" in result and "celo_integration" in result["analysis"]:
                    ...
                    if (
                        ...
                        repos_with_celo = celo_integration.get(
                            "repositories_with_celo", 0
                        )
        ...
        for idx, result in enumerate(results):
            ...
            with open(report_path, "w") as f:
                ...
                if (
                    project_url is None
                    or isinstance(project_url, float)
                    and pd.isna(project_url)
                ):
                ...
                if (
                    "project_owner_github_url" in result
                    and result["project_owner_github_url"]
                ):
                    owner_urls = result["project_owner_github_url"]
                    # Clean up the URLs (remove any string formatting artifacts)
                    clean_urls = []
                    ...
                    if len(clean_urls) == 1:
                        f.write(
                            f"**Project Developer:** [{clean_urls[0]}]({clean_urls[0]})\n\n"
                        )
                ...
                if "github_urls" in result and result["github_urls"]:
                    ...
                    if len(github_urls) == 1:
                        f.write(
                            f"**GitHub URL:** [{github_urls[0]}]({github_urls[0]})\n\n"
                        )
                    else:
                        f.write(
                            f"**GitHub URLs:** ({len(github_urls)} repositories)\n\n"
                        )
                ...
                if "analysis" in result and "repo_details" in result["analysis"]:
                    ...
                    if isinstance(repo_details, list) and len(repo_details) > 0:
                        ...
                        if len(repo_details) == 1:
                            ...
                            f.write(
                                f"- **Last Updated:** {repo.get('last_update', 'N/A')}\n\n"
                            )
                        else:
                            # Multiple repositories - create a summary table
                            f.write(
                                "| Repository | Stars | Forks | Issues | Language | Last Updated |\n"
                            )
                            f.write(
                                "|------------|-------|-------|--------|----------|-------------|\n"
                            )
                            ...
                            for repo in repo_details:
                                ...
                                f.write(
                                    f"| {repo_name} | {stars} | {forks} | {issues} | {language} | {last_update} |\n"
                                )
                            f.write("\n")
                        ...
                        f.write(
                            f"- **Open Issues:** {repo_details.get('open_issues', 'N/A')}\n"
                        )
                        f.write(
                            f"- **Primary Language:** {repo_details.get('language', 'N/A')}\n"
                        )
                        f.write(
                            f"- **Last Updated:** {repo_details.get('last_update', 'N/A')}\n\n"
                        )
                    ...
                    if isinstance(quality, (int, float)):
                        ...
                        if "error" in quality:
                            ...
                            if isinstance(overall_score, (int, float)):
                                if repos_analyzed > 1:
                                    f.write(
                                        f"**Overall Score:** {overall_score:.2f}/100 (average of {repos_analyzed} repositories)\n\n"
                                    )
                                else:
                                    f.write(
                            ...
                            # For repositories with repositories_analyzed value, we might not have detailed metrics
                            # Only try to extract them if they exist or might exist in a nested response

                            # Try to find readability score
                            readability = None
                            if "readability" in quality:
                                readability = quality.get("readability", 0)
                            elif (
                                "ai_analysis" in quality
                                and "readability" in quality.get("ai_analysis", {})
                            ):
                                readability_data = quality["ai_analysis"]["readability"]
                                if (
                                    isinstance(readability_data, dict)
                                    and "score" in readability_data
                                ):
                                    readability = readability_data["score"]
                            ...
                            if (
                                "ai_analysis" in quality
                                and "readability_reasoning"
                                in quality.get("ai_analysis", {})
                            ):
                                readability_reasoning = quality["ai_analysis"][
                                    "readability_reasoning"
                                ]

                            if isinstance(readability, (int, float)):
                                f.write(
                                    f"**Readability and Documentation:** {readability:.2f}/100\n\n"
                                )
                                if readability_reasoning:
                                    f.write(f"{readability_reasoning}\n\n")
                            else:
                                f.write(
                                    f"**Readability and Documentation:** Not evaluated\n\n"
                                )

                            # Standards score and analysis
                            standards = None
                            standards_reasoning = None
                            if "standards" in quality:
                                standards = quality.get("standards", 0)
                            elif (
                                "ai_analysis" in quality
                                and "standards" in quality.get("ai_analysis", {})
                            ):
                                standards_data = quality["ai_analysis"]["standards"]
                                if (
                                    isinstance(standards_data, dict)
                                    and "score" in standards_data
                                ):
                                    standards = standards_data["score"]

                            if (
                                "ai_analysis" in quality
                                and "standards_reasoning"
                                in quality.get("ai_analysis", {})
                            ):
                                standards_reasoning = quality["ai_analysis"][
                                    "standards_reasoning"
                                ]

                            if isinstance(standards, (int, float)):
                                f.write(
                                    f"**Coding Standards and Best Practices:** {standards:.2f}/100\n\n"
                                )
                                if standards_reasoning:
                                    f.write(f"{standards_reasoning}\n\n")
                            else:
                                f.write(
                                    f"**Coding Standards and Best Practices:** Not evaluated\n\n"
                                )
                            ...
                            if "complexity" in quality:
                                ...
                                "ai_analysis" in quality
                                and "complexity" in quality.get("ai_analysis", {})
                            ):
                                complexity_data = quality["ai_analysis"]["complexity"]
                                if (
                                    isinstance(complexity_data, dict)
                                    and "score" in complexity_data
                                ):
                                    complexity = complexity_data["score"]

                            if (
                                "ai_analysis" in quality
                                and "complexity_reasoning"
                                in quality.get("ai_analysis", {})
                            ):
                                complexity_reasoning = quality["ai_analysis"][
                                    "complexity_reasoning"
                                ]

                            if isinstance(complexity, (int, float)):
                                f.write(
                                    f"**Algorithm Complexity and Efficiency:** {complexity:.2f}/100\n\n"
                                )
                                if complexity_reasoning:
                                    f.write(f"{complexity_reasoning}\n\n")
                            else:
                                f.write(
                                    f"**Algorithm Complexity and Efficiency:** Not evaluated\n\n"
                                )
                            ...
                            elif "ai_analysis" in quality and "testing" in quality.get(
                                "ai_analysis", {}
                            ):
                                testing_data = quality["ai_analysis"]["testing"]
                                if (
                                    isinstance(testing_data, dict)
                                    and "score" in testing_data
                                ):
                                    testing = testing_data["score"]

                            if (
                                "ai_analysis" in quality
                                and "testing_reasoning"
                                in quality.get("ai_analysis", {})
                            ):
                                testing_reasoning = quality["ai_analysis"][
                                    "testing_reasoning"
                                ]

                            if isinstance(testing, (int, float)):
                                f.write(
                                    f"**Testing and Coverage:** {testing:.2f}/100\n\n"
                                )
                            ...
                            if "ai_analysis" in quality:
                                if "overall_analysis" in quality["ai_analysis"]:
                                    overall_analysis = quality["ai_analysis"][
                                        "overall_analysis"
                                    ]
                        ...
                        # We've moved the AI analysis section to be integrated with each metric above
                ...
                if "analysis" in result and "celo_integration" in result["analysis"]:
                    ...
                    if isinstance(celo, bool):
                        ...
                        if "error" in celo:
                            ...
                            f.write(
                                f"**Integrated with Celo:** {integration_status}\n\n"
                            )

                            if (
                                integrated
                                and "evidence" in celo
                                and isinstance(celo["evidence"], list)
                            ):
                                f.write("### Evidence of Integration\n\n")
                                for evidence in celo["evidence"]:
                                    if (
                                        isinstance(evidence, dict)
                                        and "keyword" in evidence
                                        and "file" in evidence
                                    ):
